...
export default function Index__Nav() {
  ...
  useEffect(()=>{
    ...
    const nav__scrollHandler = (e) => {
      ...
      if (e.target.scrollingElement.scrollTop === 0) nav.classList.remove('index__nav--scroll');
      else nav.classList.add('index__nav--scroll');
    };
  ...
  */
  ...
  const asideBox__clickHandler = (e) => {
    let start = Date.now();
    let nav__menu = document.getElementById('index__nav__menu');
    if (e.target.id === 'index__nav__rightBox__menu') {
  ...
  return (
    <nav className='index__nav'>
      <div className='index__nav__homeLink'>
        <a href='/'>
          <img className='index__homeLink__img' src='/static/nav_logo_tr(w)(100x50).svg' alt='茂洋烏魚子'></img>
        </a>
      </div>
      <Nav__centerBox 
      ...
      <div className='index__shoppingCart'></div>
    </nav>
  )
}
